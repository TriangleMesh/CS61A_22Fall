...
class HungryAnt(Ant):
    ...
    First, check if it is chewing; if so, decrement its turns_to_chew. 
    Otherwise, eat a random Bee in its place by reducing the Bee's health to 0. 
    Make sure to set the turns_to_chew when a Bee is eaten!
    """
    def action(self, gamestate):
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container == False and self.ant_contained is None:
            return True
        else:
            return False
        # END Problem 8a
