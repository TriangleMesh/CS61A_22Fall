...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # then add the current environment env as the last argument to this Python list.
        if procedure.need_env is True:
            lst += [env]
        # Return the result of calling procedure.py_func on all of those arguments. 
        # Use *args notation: f(1, 2, 3) is equivalent to f(*[1, 2, 3]). 
        # Do this part within the try statement provided, after the line that says try:.
        try:
            # BEGIN PROBLEM 2
...
def eval_all(expressions, env):
    ...
    if expressions.rest is nil:
        return result
    # 为什么要额外提出result，不能直接return scheme_eval(expressions.first, env)? 
    else:
