...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def f(test_words):
        test_words2 = split(remove_punctuation(lower(test_words)))
        for i in range(len(test_words2)):
            if test_words2[i] in topic:
                return True
        return False
    return f
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        error_list.append(diff_function(typed_word, word_list[i], limit))
...
def feline_fixes(typed, source, limit):
    ...
    def f(typed, source, n, limit):
        if n >= min(len(typed), len(source)):
            return abs(len(typed)- len(source))
        if limit == 0:
            return int(typed[n:] != source[n:])
        return f(typed, source, n+1, limit - int(typed[n] != source[n])) + int(typed[n] != source[n])
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def count(typed, prompt):
        if len(typed) == 0 or typed[0] != prompt[0]:
            return 0
        else:
            return count(typed[1:], prompt[1:]) + 1
    ratio = count(typed, prompt) / len(prompt)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    time_list = get_all_times(match) # [[5, 1, 3], [4, 1, 6]]
    word_list = get_all_words(match) # ['Just', 'have', 'fun']
    lst = [[] for i in player_indices]
    for i, word in enumerate(word_list):
        # word_times = [time_list[player][i] for player in player_indices]
        word_times = [time(match, player, i) for player in player_indices]
        # print(word_times)
