...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        # 通过构造LambdaProcedure class，然后把 f = lambda...
        lambda_exprs = Pair(signature.rest, expressions.rest)
        env.define(signature.first, do_lambda_form(lambda_exprs, env))
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil or is_scheme_false(result):
        return result
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    result = scheme_eval(expressions.first, env)
    if expressions.rest is nil or is_scheme_true(result):
        return result
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        signature = binding.first
        if scheme_symbolp(signature):
            names = Pair(signature, names)
