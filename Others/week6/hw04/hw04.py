...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return left_length * left_weight == right_length * right_weight and balanced(end(left(m))) and balanced(end(right(m)))
...
def totals_tree(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_mobile(m):
        return [total_weight(m)] + list([totals_tree(end(left(m))), totals_tree(end(right(m)))])
    else:
