def flatten(s):
    ...
    def change(s):
        ...
        for i in range(len(s)):
            if type(s[i]) != list:
                ...
                lst.append(s[i])
        return lst
    # determine whether all the items are numbers
    def is_num(s):    
        for i in range(len(s)):
            if type(s[i]) == list:
                return False
    ...
    add_list = change(s)
    combine_list = sum(add_list, [])
    if is_num(combine_list):
        return combine_list
    else:
        return flatten(combine_list)   
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city = make_city('city_name', lat, lon)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) or is_leaf(t2):
        ...
        for m in range(min(len(branches(t1)), len(branches(t2))), max(len(branches(t1)), len(branches(t2)))):
            if len(branches(t1)) > len(branches(t2)):
                new_branches += [branches(t1)[m]]
            else:
                new_branches += [branches(t2)[m]]
