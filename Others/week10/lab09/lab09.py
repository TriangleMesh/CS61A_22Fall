...
def subseqs(s):
    ...
    if not s:
        ...
        num = s.pop(0)
        temp = subseqs(s)
        return insert_into_all(num, temp) + temp
    # 联系partition，把答案拆成两部分，比如有1的和无1的。1则由num去pop
...
# 原理类似subseq(s)，也是将其分为两部分，例如a中是有1的，而b中是无1的，实现有1的方法是将所有的1加入到a的前面
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    # 如果要构成n个leaves，则左右两边的二叉树的树叶量加起来总和为 n = (n-i) + i，
    # 左边有n-i个树叶的二叉树有f(n)种可能的模样，右边的是f(n-i)种可能性。
    # 同时n的加法排列也有 2 * (n-1) 种可能性，需要将每种可能性相加
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            self.balance = 0
            return f'Here is your {self.name} and ${self.balance - self.price} change.'
        elif self.balance == self.price and self.stock > 0:
            self.balance = 0
            self.stock -= 1
...
def trade(first, second):
    ...
    while m <= len(first) and n <= len(second) and not equal_prefix():
        if sum(first[:m]) < sum(second[:n]):
            ...
            n += 1
# 如果不加入 *and not equal_prefix()* 则while会持续循环，而不会在满足条件的时候脱离循环
    if equal_prefix():
...
def insert(link, value, index):
    ...
    # 同时assign value而不存在先后顺序，即后面那个值用前面那个值的最初的值而不是变化后的值来assign
    elif link.rest is Link.empty or index < 0:
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif isinstance(lnk.first, Link) and lnk.rest is not Link.empty:
        return deep_len(lnk.first) + deep_len(lnk.rest)
    elif isinstance(lnk.first, int) and lnk.rest is not Link.empty:
        return 1 + deep_len(lnk.rest)
    elif isinstance(lnk.first, int) and lnk.rest is Link.empty:
        return 1
    # 这种做法更简洁，因为原本就是使用加法相连接（将前面一半和后面一半加起来）
    # 不用特意筛选是否lnk.rest为Link.empty，因为lnk.rest会作为lnk去被判断
    # 
    # 标准答案：
    # if lnk is Link.empty:
    #     return 0
...
def long_paths(t, n):
    ...
    if t.is_leaf() and n <= 0:
        return [[t.label]]
    for b in t.branches:
        paths += [ [t.label] + path for path in long_paths(b, n-1) ]
    return paths
# 这是一道两个for循环的结构，把符合条件的通过if挑选出来（即t为叶子——到了最后一个数，且此时n已经逐步扣减到了0）
# 对于每一个有树杈的t都要走一遍
...
def reverse_other(t):
    ...
    n = len(t.branches)
    if n > 1:
        new_labs = [b.label for b in t.branches][::-1] # [::-1]可以直接倒序
        for i in range(n):
